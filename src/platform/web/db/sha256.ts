/* ==========================================================================
   ...
   The seed generator hashes demo PINs while it builds rows, so it needs a
   synchronous hasher; WebCrypto's digest() is async. This keeps the browser
   seeder byte-identical to the desktop's node:crypto path, so a database
   seeded in either runtime accepts the same PINs.

   Verified against node:crypto in tests/platform/sha256.test.ts.
   ========================================================================== */
...
/** Same contract as the desktop's hashPin (node:crypto sha256 of `salt:pin`). */
